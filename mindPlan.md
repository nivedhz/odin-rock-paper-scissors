...
* In a function compare the values and return 
  * 1)invalid-if no input is prevailed- 
  * 2)draw-if the user and computer input are the same- 
  * 3)loss - if the user's input matches with the key of computerBeats and the computer's input also matches with the value of the computerBeats. 
  * 4)win- if any of this doesn't happen.
